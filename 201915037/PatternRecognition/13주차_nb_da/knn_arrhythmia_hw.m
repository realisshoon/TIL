...
% TR_LABEL    = [2*ones(length(tr_RMSSD_af), 1); ones(length(tr_RMSSD_n), 1)];
...
% TS_LABEL    = [2*ones(length(ts_RMSSD_af), 1); ones(length(ts_RMSSD_n), 1)];
% figure;
...
%%
Dist={};
Dist={'cityblock','chebychev','correlation','cosine','mahalanobis','minkowski','euclidean'};
cnt=1; %count 변수
kj_set=[];
acc_set=[];
...
for k=1:1:3
    ...
    kj_set(cnt,1)=k; 
    kj_set(cnt,2)=j; %cityblock 일 때 accuracy 최대
...
[mv,mi]=max(acc_set) %[가장 높은 value, 가장 높은 index]
